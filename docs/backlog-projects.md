...
- [x] **P-6: Fix tie-breaker state on decline.** → **DONE (2026-07-16).**
      ...
      _before_ the sub-round, keeping `resolveTieBreaker`'s tail-slice math
...
- [x] **P-7: Split `RoomState` (1216 loc, four responsibilities).** →
      **DONE (2026-07-16).** The FLIP/card-fly cluster moved to
      `cardTransitions.svelte.ts` (`CardTransitions` class, wired as
      `roomState.transitions`, mirroring `CardDragState`): `cardIn`/`cardOut`,
      `captureCardRects`, rect maps, reconnect/new-card tracking (the
      `onStateReceived` pre-assignment hook replaces ~35 inline loc in the WS
      handler). RoomState is 873 loc and animation-free. Autoplay now reuses
      shared play-finders: `getLegalPlays` for phase 2 (so autoplay can finally
      play runs) and a new `getLegalPlaysPhase1` in `shared/rules.ts` (with
      tests appended to `shared/tests/rules.test.ts`) — the powerset
      enumeration is gone. Verified end-to-end: two-browser autoplay game ran
      phase 1 → 2 → Skitgubbe poster with zero server rejections (recipe in
      `.claude/skills/verify/SKILL.md`). Note: dev-mode SSR 500s on
      `@dotlottie/player-component` (`window is not defined`) — pre-existing,
      untouched here.
